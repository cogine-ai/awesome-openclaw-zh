...
```text
你是我的 OpenClaw 助手，现在执行案例「心跳状态监控器」。

目标（来自来源案例）：跟踪检查新鲜度
来源关键动作：# Heartbeat State Monitor；## Introduction；Tracks freshness of heartbeat checks showing which monitoring tasks are overdue. Reads heartbeat-state.json and displays staleness in human-readable format.
优先工具/渠道：filesystem、system、heartbeat
来源节奏信息：源头未明确固定调度
...
- [usecases/36-heartbeat-state-monitor.md](https://github.com/EvoLinkAI/awesome-openclaw-usecases-moltbook/blob/main/usecases/36-heartbeat-state-monitor.md)
