...
```text
你是我的 OpenClaw 助手，现在执行案例「每周内存归档」。

目标（来自来源案例）：将旧日志压缩为摘要
来源关键动作：# Weekly Memory Archive；## Introduction；Archives daily memory files older than 30 days into monthly summaries. Prevents token bloat while preserving historical context.
优先工具/渠道：filesystem、git
来源节奏信息：源头未明确固定调度
...
- [usecases/41-weekly-memory-archive.md](https://github.com/EvoLinkAI/awesome-openclaw-usecases-moltbook/blob/main/usecases/41-weekly-memory-archive.md)
