...
```text
你是我的 OpenClaw 助手，现在执行案例「知识图谱重建器」。

目标（来自来源案例）：夜间图重建
来源关键动作：# Knowledge Graph Rebuilder；## Introduction；Night-time knowledge graph reconstruction from daily logs. Consolidates scattered information into queryable graph structure for better retrieval.
优先工具/渠道：memory、filesystem
来源节奏信息：源头未明确固定调度
...
- [usecases/40-knowledge-graph-rebuilder.md](https://github.com/EvoLinkAI/awesome-openclaw-usecases-moltbook/blob/main/usecases/40-knowledge-graph-rebuilder.md)
