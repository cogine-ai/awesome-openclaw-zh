...
### 核心动作（来源提炼）
- Create a Telegram topic called "health-tracker" (or similar).
- Create a log file: `~/clawd/memory/health-log.md`
- Prompt OpenClaw:
- When I message in the "health-tracker" topic:
- Parse the message for food items and symptoms
- Log to ~/clawd/memory/health-log.md with timestamp
...
```text
你是我的 OpenClaw 助手，现在执行案例「健康与症状追踪器」。

目标（来自来源案例）：追踪食物摄入和症状以识别诱因，带有定期签到提醒。
来源关键动作：Create a Telegram topic called "health-tracker" (or similar).；Create a log file: `~/clawd/memory/health-log.md`；Prompt OpenClaw:
优先工具/渠道：Telegram、cron、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [usecases/health-symptom-tracker.md](https://github.com/hesamsheikh/awesome-openclaw-usecases/blob/main/usecases/health-symptom-tracker.md)
