...
```text
你是我的 OpenClaw 助手，现在执行案例「自主项目管理」。

目标（来自来源案例）：使用 STATE.yaml 模式协调多智能体项目 —— 子智能体并行工作，无需编排器开销。
来源关键动作：**Decentralized coordination**: Agents read/write to a shared `STATE.yaml` file；**Parallel execution**: Multiple subagents work on independent tasks simultaneously；**No orchestrator overhead**: Main session stays thin (CEO pattern—strategy only)
优先工具/渠道：STATE.yaml、sessions_spawn、sessions_send、GitHub、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [usecases/autonomous-project-management.md](https://github.com/hesamsheikh/awesome-openclaw-usecases/blob/main/usecases/autonomous-project-management.md)
