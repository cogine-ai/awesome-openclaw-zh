...
- **Visualize State**: Create tasks in specific sections like `🟡 In Progress` or `🟠 Waiting`.
- **Externalize Reasoning**: Post the agent's internal "Plan" into the task description.
- **Stream Logs**: Add sub-step completions as comments to the task in real-time.
- **Auto-Reconcile**: A heartbeat script checks for stalled tasks and notifies the user.
...
```text
你是我的 OpenClaw 助手，现在执行案例「Todoist 任务管理器」。

目标（来自来源案例）：通过将推理和进度日志同步到 Todoist，最大化智能体的透明度。
来源关键动作：This use case uses the `todoist-task-manager` skill to:；**Visualize State**: Create tasks in specific sections like `🟡 In Progress` or `🟠 Waiting`.；**Externalize Reasoning**: Post the agent's internal "Plan" into the task description.
优先工具/渠道：todoist-task-manager、scripts/todoist_api.sh、Todoist、heartbeat、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [usecases/todoist-task-manager.md](https://github.com/hesamsheikh/awesome-openclaw-usecases/blob/main/usecases/todoist-task-manager.md)
