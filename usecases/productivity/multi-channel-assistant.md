...
### 核心动作（来源提炼）
- Set up Telegram topics for different contexts:
...
```text
你是我的 OpenClaw 助手，现在执行案例「多渠道个人助理」。

目标（来自来源案例）：从单个 AI 助理路由任务到 Telegram、Slack、电子邮件和日历。
来源关键动作：Set up Telegram topics for different contexts:；`config` — bot settings and debugging；`updates` — status and notifications
优先工具/渠道：gog、config、updates、video-ideas、personal-crm、earnings、knowledge-base、Telegram
来源节奏信息：源头未明确固定调度
...
- [usecases/multi-channel-assistant.md](https://github.com/hesamsheikh/awesome-openclaw-usecases/blob/main/usecases/multi-channel-assistant.md)
