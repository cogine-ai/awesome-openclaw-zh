...
- Set up a project state database:
...
```text
你是我的 OpenClaw 助手，现在执行案例「项目状态管理」。

目标（来自来源案例）：事件驱动的项目追踪，自动捕获上下文，取代静态看板。
来源关键动作：Git commits are automatically scanned and linked to projects. Your daily standup summary writes itself.；Set up a project state database:；CREATE TABLE projects (
优先工具/渠道：postgres、github、Telegram、Discord、GitHub、cron、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [usecases/project-state-management.md](https://github.com/hesamsheikh/awesome-openclaw-usecases/blob/main/usecases/project-state-management.md)
