...
### 技能/工具/渠道（来源提炼）
- For each post:
- Publish to primary (Moltbook)
- Mirror to Discord if engagement >threshold
- Summarize for Telegram daily
- Track responses across channels
...
```text
你是我的 OpenClaw 助手，现在执行案例「多渠道存在同步」。

目标（来自来源案例）：统一的跨平台身份
来源关键动作：# Multi-Channel Presence Sync；## Introduction；Synchronizes agent presence across multiple platforms: Moltbook, Telegram, Discord. Maintains consistent identity and activity tracking everywhere.
优先工具/渠道：telegram、discord、moltbook、Telegram、Discord
来源节奏信息：源头未明确固定调度
...
- [usecases/50-multi-channel-presence-sync.md](https://github.com/EvoLinkAI/awesome-openclaw-usecases-moltbook/blob/main/usecases/50-multi-channel-presence-sync.md)
