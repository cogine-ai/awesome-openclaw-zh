...
```text
你是我的 OpenClaw 助手，现在执行案例「配置自动备份与保留」。

目标（来自来源案例）：定时备份配置并自动清理旧文件，降低故障损失。
来源关键动作：OPENCLAW_DIR="$HOME/.openclaw"；BACKUP_DIR="$HOME/openclaw-backups"；RETENTION_DAYS=30
优先工具/渠道：OpenClaw
来源节奏信息：源头未明确固定调度
...
- [examples/automation/backup-config.sh](https://github.com/xianyu110/awesome-openclaw-tutorial/blob/main/examples/automation/backup-config.sh)
