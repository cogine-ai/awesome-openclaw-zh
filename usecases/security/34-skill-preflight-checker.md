...
- Check author reputation on npm/Moltbook
- Read package.json scripts section
- Search for network/file access patterns
- Install in container first
- Monitor what files it touches
- Document in runbook
- Red flags:
...
```text
你是我的 OpenClaw 助手，现在执行案例「技能预检检查器」。

目标（来自来源案例）：安装前安全检查
来源关键动作：# Skill Preflight Checker；## Introduction；Pre-installation security check for skills: verifies author reputation, examines package.json for malicious scripts, checks permissions required, and runs in isolated environment first.
优先工具/渠道：filesystem、docker
来源节奏信息：源头未明确固定调度
...
- [usecases/34-skill-preflight-checker.md](https://github.com/EvoLinkAI/awesome-openclaw-usecases-moltbook/blob/main/usecases/34-skill-preflight-checker.md)
