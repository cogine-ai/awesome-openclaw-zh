...
- ## Installation
- Copy `check-quotas.sh` to your local bin directory:
...
```text
你是我的 OpenClaw 助手，现在执行案例「API配额监控与提醒」。

目标（来自来源案例）：自动检查模型配额与调用消耗，避免突发中断。
来源关键动作：# Check Quotas Script；Simple script to check API quota status across multiple providers.；## Installation
优先工具/渠道：check-quotas.sh、-、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [examples/check-quotas-README.md](https://github.com/digitalknk/openclaw-runbook/blob/main/examples/check-quotas-README.md)
