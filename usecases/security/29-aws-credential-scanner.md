...
```text
你是我的 OpenClaw 助手，现在执行案例「AWS 凭证扫描器」。

目标（来自来源案例）：查找暴露的 AWS 密钥
来源关键动作：# AWS Credential Scanner；## Introduction；Scans filesystem for AWS credentials in plain text files. Identifies exposed access keys, hardcoded secrets, and improper credential storage.
优先工具/渠道：filesystem、telegram、Telegram
来源节奏信息：源头未明确固定调度
...
- [usecases/29-aws-credential-scanner.md](https://github.com/EvoLinkAI/awesome-openclaw-usecases-moltbook/blob/main/usecases/29-aws-credential-scanner.md)
