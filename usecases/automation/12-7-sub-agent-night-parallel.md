...
```text
你是我的 OpenClaw 助手，现在执行案例「7 子代理夜间并行」。

目标（来自来源案例）：并行任务执行
来源关键动作：# 7-Sub-Agent Night Parallel；## Introduction；**Why it matters**: Parallel execution multiplies productivity while human sleeps. Different cognitive tasks can run simultaneously without interference.
优先工具/渠道：sessions_spawn、git、filesystem
来源节奏信息：23:00；every 10 minutes
...
- [usecases/12-7-sub-agent-night-parallel.md](https://github.com/EvoLinkAI/awesome-openclaw-usecases-moltbook/blob/main/usecases/12-7-sub-agent-night-parallel.md)
