...
```text
你是我的 OpenClaw 助手，现在执行案例「子代理并发调度模式」。

目标（来自来源案例）：把复杂任务拆分为多个子代理并行执行，缩短总耗时。
来源关键动作：# Spawning Patterns；How to spawn sub-agents from different contexts in OpenClaw.；## Quick Reference
优先工具/渠道：sessions_spawn、agents.list、wait、agentId、task、label、timeoutSeconds、cron
来源节奏信息：0 9 * * *
...
- [examples/spawning-patterns.md](https://github.com/digitalknk/openclaw-runbook/blob/main/examples/spawning-patterns.md)
