...
```text
你是我的 OpenClaw 助手，现在执行案例「定时任务仪表板状态」。

目标（来自来源案例）：可视化所有定时任务
来源关键动作：# Cron Dashboard Status；## Introduction；Centralized monitoring dashboard for all cron jobs showing last run time, success/failure status, next scheduled run, and recent output summaries.
优先工具/渠道：system、filesystem、cron
来源节奏信息：源头未明确固定调度
...
- [usecases/35-cron-dashboard-status.md](https://github.com/EvoLinkAI/awesome-openclaw-usecases-moltbook/blob/main/usecases/35-cron-dashboard-status.md)
