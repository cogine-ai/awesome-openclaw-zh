...
### 核心动作（来源提炼）
- Set up a Telegram topic for video ideas and configure it in OpenClaw.
- Install the knowledge-base skill and x-research skill.
- Create a SQLite database for pitch tracking:
...
```text
你是我的 OpenClaw 助手，现在执行案例「YouTube 内容流水线」。

目标（来自来源案例）：为 YouTube 频道自动化视频创意发掘、研究和追踪。
来源关键动作：Set up a Telegram topic for video ideas and configure it in OpenClaw.；Install the knowledge-base skill and x-research skill.；Create a SQLite database for pitch tracking:
优先工具/渠道：web_search、gog、Telegram、Slack、Todoist、cron、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [usecases/youtube-content-pipeline.md](https://github.com/hesamsheikh/awesome-openclaw-usecases/blob/main/usecases/youtube-content-pipeline.md)
