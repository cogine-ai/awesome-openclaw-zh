...
```text
你是我的 OpenClaw 助手，现在执行案例「每周技术发现精选」。

目标（来自来源案例）：自动追踪和筛选技术动态，输出可读性高的周报。
来源关键动作：[ ] Newsletter subscriptions (or RSS feeds as alternative)；[ ] Email access (IMAP/SMTP) or RSS reader；[ ] Web search tool configured (Brave, Serper API key)
优先工具/渠道：cron.jobs、Telegram、Discord、Slack、Todoist、GitHub、cron、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [showcases/tech-discoveries.md](https://github.com/digitalknk/openclaw-runbook/blob/main/showcases/tech-discoveries.md)
