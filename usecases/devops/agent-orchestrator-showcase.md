...
```text
你是我的 OpenClaw 助手，现在执行案例「编码任务编排中心（Agent Orchestrator）」。

目标（来自来源案例）：把编码任务路由给最合适的工具链，提升多任务效率。
来源关键动作：[ ] Multiple CLI coding tools installed (claude, codex, opencode, gemini)；[ ] API keys for each provider in `~/.openclaw/credentials/`；[ ] Understanding of each tool's strengths
...
- [showcases/agent-orchestrator.md](https://github.com/digitalknk/openclaw-runbook/blob/main/showcases/agent-orchestrator.md)
