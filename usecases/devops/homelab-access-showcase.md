...
```text
你是我的 OpenClaw 助手，现在执行案例「家庭实验室远程安全访问」。

目标（来自来源案例）：通过聊天工具安全触发远程运维操作，降低误操作风险。
来源关键动作：[ ] Tailscale (or WireGuard, ZeroTier) installed on all devices；[ ] SSH keys configured and authorized on devices；[ ] Telegram bot created via @BotFather
优先工具/渠道：/newbot、Telegram、Discord、Slack、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [showcases/homelab-access.md](https://github.com/digitalknk/openclaw-runbook/blob/main/showcases/homelab-access.md)
