...
- Fetch the content (article, tweet, YouTube transcript, PDF)
- Ingest it into the knowledge base with metadata (title, URL, date, type)
...
```text
你是我的 OpenClaw 助手，现在执行案例「个人知识库 (RAG)」。

目标（来自来源案例）：通过将 URL、推文和文章拖入聊天来构建可搜索的知识库。
来源关键动作：Install the knowledge-base skill from ClawdHub.；Create a Telegram topic called "knowledge-base" (or use a Slack channel).；Prompt OpenClaw:
优先工具/渠道：web_fetch、Telegram、Slack、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [usecases/knowledge-base-rag.md](https://github.com/hesamsheikh/awesome-openclaw-usecases/blob/main/usecases/knowledge-base-rag.md)
