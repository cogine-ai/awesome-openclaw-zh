...
```text
你是我的 OpenClaw 助手，现在执行案例「语义记忆搜索」。

目标（来自来源案例）：使用混合检索和自动同步，为你的 OpenClaw markdown 记忆文件添加向量驱动的语义搜索。
来源关键动作：Index all your OpenClaw markdown memory files into a vector database (Milvus) with a single command；Search by meaning: "what caching solution did we pick?" finds the relevant memory even if the word "caching" does not appear；Hybrid search (dense vectors + BM25 full-text) with RRF reranking for best results
优先工具/渠道：index、GitHub、OpenClaw
来源节奏信息：源头未明确固定调度
...
- [usecases/semantic-memory-search.md](https://github.com/hesamsheikh/awesome-openclaw-usecases/blob/main/usecases/semantic-memory-search.md)
